...
public class JChatMuteCommand implements CommandExecutor {

    private final Plugin plugin;

    public JChatMuteCommand(Plugin plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        if (!(commandSender.hasPermission("jchat.jcmute") || commandSender.isOp())) {
            ...
            if (strings.length != 0) {
                if (!p.isUsernameInArray(players, strings[0])) {
                    ...
                    if(muted) {
                        commandSender.sendMessage( strings[0] +  " is already currently muted.");
                        return true;
                    }
                    else {
                        p.UpdatePlayerData(players, String.valueOf(uuid), "isMuted", true);
                        commandSender.sendMessage(ChatColor.GREEN + strings[0] +  " was muted.");
                        return true;
                    }
                }
            } else {
